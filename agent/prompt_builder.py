...
# The skills-index renderer + its scan/cache helpers now live in the shared
# leaf module ``agent.skills_index`` so the engine runtime and the standalone
# index generator render a byte-identical block from one source of truth.
# Re-exported here for backward compatibility — existing imports of these
# names from ``agent.prompt_builder`` (tests, hermes_cli, tools) keep working.
from agent.skills_index import (  # noqa: F401  (re-exported)
...
def build_skills_system_prompt(
    ...
    """
    # Thin wrapper around the shared single-source renderer. The LRU +
    # disk-snapshot cache and the full scan/render logic live in
    # ``agent.skills_index.render_skills_index`` so the engine runtime and the
    # standalone index generator produce a byte-identical block.
    return render_skills_index(
